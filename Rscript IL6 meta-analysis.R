...
Axelsson2013ind$IL6_pgml.1[Axelsson2013ind$IL6_pgml.1 < 0.9] <- 0.9 # Values below detection limit set to detection limit - there were none
...
Model1.5 <- lme(lnIL6 ~ SinHour + CosHour + SinHour12 + CosHour12  + SinHour6 + CosHour6  + SinHour3 + CosHour3  + SinHour1.5 + CosHour1.5 + TimeFromCatheter_h, data = IL6_data, weights = ~ 1/sqw, random = ~ TimeFromCatheter_h|Study, na.action = na.omit)
summary(Model1.5)
...
cols <- colorRampPalette(brewer.pal(4,"Set1"))(3)

pdf("C:/Users/FIXITx64_ENG/Dropbox/Gustavs_arbete/Pek/IL-6 review/Fig_summary_pred.pdf")
...
axis(side = 1, at = c(0, 6, 12, 18, 24), labels = c("00:00", "06:00", "12:00", "18:00", "24:00"), cex.axis = 1.5)
axis(2, at = c(0, 0.5, 1), labels = c(0, 0.5, 1), cex.axis = 1.5)
lines(Intercept + sinPred + cosPred, lwd = 2, col = "red", lty = 2)
...
for (i in unique(IL6_data$Study)){
  with(IL6_data[IL6_data$Study == i, ],
       plotCI(Hour, lnIL6, IL6_sd, main = i, ylab = "ln IL-6, pg/ml", frame.plot = F)
  )
  lines(predict(Model24)[IL6_data$Study == i] ~ IL6_data[IL6_data$Study == i, ]$Hour, col = "red", lty = 2)
  lines(predict(Model12)[IL6_data$Study == i] ~ IL6_data[IL6_data$Study == i, ]$Hour, col = "blue")
}

# Model variances
Model24sd <- lme(IL6_sd ~ SinHour + CosHour, data = IL6_data, weights = ~ 1/sqw, random = ~ 1|Study)
summary(Model24sd)
plot(predict(Model24sd) ~ IL6_data[IL6_data$Study != "Born1997", ]$Hour)

# Investigate effect of sleep
ModelSleep <- lme(IL6_pgml.1 ~ SinHour + CosHour + TimeFromCatheter_h + Sleep, data = BaselineData, weights = ~ 1/n, random = ~ TimeFromCatheter_h|Study)
summary(ModelSleep)
plot(predict(ModelSleep) ~ BaselineData$Hour)
acrophaseSleep <- -atan(-ModelSleep$coefficients$fixed["SinHour"]/ModelSleep$coefficients$fixed["CosHour"])*24/(2*pi) #Acrophase
amplitudeSleep <- sqrt(ModelSleep$coefficients$fixed["SinHour"]^2 + ModelSleep$coefficients$fixed["CosHour"]^2) #Amplitude
lines(ModelSleep$coefficients$fixed["(Intercept)"] + ModelSleep$coefficients$fixed["SinHour"]*sin(2*pi*(1:24)/24) + ModelSleep$coefficients$fixed["CosHour"]*cos(2*pi*(1:24)/24), lwd = 2, col = "orange")

# Investigate effect of sleep deprivation
Model24full <- lme(IL6_pgml.1 ~ SinHour + CosHour + TimeFromCatheter_h, data = IL6_data, weights = ~ 1/n, random = ~ TimeFromCatheter_h|Study)
summary(Model24full)
plot(predict(Model24full) ~ IL6_data$Hour)

Model24fullDeprivation <- lme(IL6_pgml.1 ~ SinHour + CosHour + TimeFromCatheter_h + Deprived, data = IL6_data, weights = ~ 1/n, random = ~ TimeFromCatheter_h|Study)
summary(Model24fullDeprivation)
plot(predict(Model24fullDeprivation) ~ IL6_data$Hour)

# Investigate time from catheterization effect
ranef <- ranef(Model12)
boxplot(ranef$TimeFromCatheter_h[ranef$TimeFromCatheter_h < 0.1 & ranef$TimeFromCatheter_h > -0.1], frame.plot = F, ylim = c(-0.25, 0.2))
...
########################################
# ANALYSE INFLUENTIAL STUDY (MEHRA 2006)
########################################
MehraData <- subset(BaselineData, BaselineData$Study %in% c("Mehra2006a", "Mehra2006b", "Mehra2006c", "Mehra2006d", "Mehra2006e"))
MehraData$TimePoint <- NA
MehraData$TimePoint[MehraData$Hour == 31.5] <- "Morning"
MehraData$TimePoint[MehraData$Hour == 22.5] <- "Evening"

# Build models
Model24_Mehra <- lme(lnIL6 ~ TimePoint, data = MehraData, weights = ~ 1/sqw, random = ~ 1|Study)
summary(Model24_Mehra)
plot(Model24_Mehra, resid(., type = "p") ~ Hour, abline = 0)
plot(Model24_Mehra, resid(., type = "p") ~ fitted(.), abline = 0)
intervals(Model24_Mehra)
